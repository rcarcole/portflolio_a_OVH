...
const catName = id => (CATS.find(c => c.id === id) || NOCAT).name;
// si un producto trae una categoría que ya no existe, se trata como sin categoría
const catOf = it => CATS.some(c => c.id === it.cat) ? it.cat : '';
...
let openState = readCache(CACHE_OPEN, {});
...
function touch(id, patch){
  const prev = items[id] || { name:'', qty:0, want:0, cat:'', note:'', deleted:false };
...
function render(){
  ...
  if (missing.length){
    ...
    groupsOf(missing).forEach(g => {
      mList.appendChild(groupBlock('falta', g, true, missRow));
    });
  } else {
    wrap.hidden = true;
  }
...
/* Agrupa por categoría, en el orden en que se recorre el súper.
   Solo aparecen las categorías que tienen algo. */
function groupsOf(list){
  const out = [];
  CATS.concat([NOCAT]).forEach(cat => {
    const rows = list.filter(i => catOf(i) === cat.id);
    if (rows.length) out.push({ cat, rows });
  });
  return out;
}

/* Cabecera plegable + sus productos */
function groupBlock(section, g, openByDefault, rowFn){
  ...
  head.type = 'button';
  head.className = 'group-head' + (section === 'falta' ? ' group-head-miss' : '');
  ...
  head.onclick = () => {
    openState[key] = !open;
    writeCache(CACHE_OPEN, openState);
    render();
  };

  const ul = document.createElement('ul');
  ul.className = section === 'falta' ? 'miss-list' : 'rows';
  ul.hidden = !open;
  g.rows.forEach(it => ul.appendChild(rowFn(it)));

  box.append(head, ul);
  return box;
}

/* --- una fila del bloque "Falta" --- */
function missRow(it){
  ...
  less.type = 'button';
  less.textContent = '−';
  less.setAttribute('aria-label', buy <= 1
  ...
  less.onclick = () => {
    if (buy <= 1){ restock(it.id); toast(`${it.name}: vuelve a la lista`); }
    else { setWant(it.id, buy - 1); }
  };
...
function stockRow(it){
  ...
  del.type = 'button';
  del.textContent = '✕';
  del.setAttribute('aria-label', `Borrar ${it.name} de la lista`);
  del.onclick = () => {
    if (!confirm(`¿Borrar "${it.name}" de la lista?\n\nDesaparece para los dos. Si solo se ha acabado, usa el botón 0.`)) return;
    removeItem(it.id);
    toast(`${it.name}: borrado`);
  };
...
/* Selector de categoría de cada producto */
function catPicker(it){
  const sel = document.createElement('select');
  sel.className = 'cat-pick' + (catOf(it) ? '' : ' cat-none');
  sel.setAttribute('aria-label', `Categoría de ${it.name}`);

  CATS.concat([NOCAT]).forEach(c => {
    const op = document.createElement('option');
    op.value = c.id;
    op.textContent = c.id ? c.name : '— sin categoría —';
    if (c.id === catOf(it)) op.selected = true;
    sel.appendChild(op);
  });

  // mientras el desplegable está abierto no repintamos
  sel.addEventListener('focus', () => { selectOpen = true; });
  sel.addEventListener('blur',  () => { selectOpen = false; if (renderPending) render(); });
  sel.addEventListener('change', () => {
    selectOpen = false;
    touch(it.id, { cat: sel.value });
    toast(`${it.name} → ${sel.value ? catName(sel.value) : 'sin categoría'}`);
  });
